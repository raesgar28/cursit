...
<h1>Descuentos | Gestión Descuentos</h1>
<ol class="breadcrumb">
...
<li><a href="#">Listado Descuentos</a></li>
</ol>
</div>
<div class="row">
<div class="col-md-12">
...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Descuentos registrados</span></div>
<div class="actions">
<div class="btn-group">
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_descuento")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td><%=ob_rs("titulo_esp")%></td>
<td><%=ob_rs("codigo")%></td>
<td style="text-align:center"><%if ob_rs("activo")="si"then%><i class="fa fa-check f_green" aria-hidden="true"></i><span class="f_white_p">Sí</span><%else%><i class="fa fa-times f_red" aria-hidden="true"></i><span class="f_white_p">No</span><%end if%></td>
<td style="text-align:center"><%if ob_rs("d_fijo")<>0 then%> <%= FormatNumber(ccur(ob_rs("d_fijo")),2)%><%else%> <%end if%></td>
<td style="text-align:center"><%if ob_rs("d_porcentaje")<>0 then%> <%= FormatNumber(ccur(ob_rs("d_porcentaje")),2)%><%else%> <%end if%></td>
<td style="text-align:center"><%fecha_now=FormatDateTime(cdate(ob_rs("fecha_in")),2)
...
<td style="text-align:center"><%=ob_rs("canjes")%></td>
...
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_descuento")&"&cod_cat="&cod_cat))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_descuento")&"&cod_cat="&cod_cat)%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_descuento")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_descuento")%>&accion=del&cod_cat=<%=cod_cat%>';}</script>
...
var min = parseInt( $('#min').val(), 8 );
var max = parseInt( $('#max').val(), 8 );
...
"pageLength": 20,
"dom": "<'row'><'table-scrollable't><'row' <'col-md-12'>><'row'<'col-md-6 col-sm-12'l><'col-md-6 col-sm-12'f>r><'row'<'col-md-5 col-sm-12'i><'col-md-7 col-sm-12'p>>", 
});
