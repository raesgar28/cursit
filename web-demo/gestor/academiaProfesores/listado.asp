...
<h1><%if ndos="prof" then%>Profesores<%else%>Monitores<%end if%> | Gestión <%if ndos="prof" then%>Profesores<%else%>Monitores<%end if%></h1>
<ol class="breadcrumb">
...
<li><a href="#">Listado <%if ndos="prof" then%>Profesores<%else%>Monitores<%end if%></a></li>
</ol>
</div>
<div class="row">
<div class="col-md-12">
...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | <%if ndos="prof" then%>Profesores<%else%>Monitores<%end if%> Registrados</span></div>
<div class="actions" >
...
<%if ndos="prof"then
Sql= "SELECT * from academia_profesores order by nombre, apellidos" 
else
Sql= "SELECT * from extra_monitores order by nombre, apellidos"   
end if  
...
<th> Nombre</th>
<th> Teléfono </th>
<%if ndos="prof"then%><th> Idioma </th><%end if%>
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_profesor")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td><%=ob_rs("nombre")%>&nbsp;<%=ob_rs("apellidos")%> </td>
<td> <%=ob_rs("telefono")%> </td>
<%if ndos="prof"then%><td>
<%Sql2= "SELECT * from idiomas where idioma='"&ob_rs("idioma")&"'"   
...
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_profesor")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_profesor"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_profesor")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_profesor")%>&accion=del';}
</script>
...
{orderable: false, targets:   <%if ndos="prof"then%>4<%else%>3<%end if%>},//ULTIMA -> ACCIONES NO SE PUEDE ORDENAR
