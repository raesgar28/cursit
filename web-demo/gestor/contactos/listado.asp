...
<h1>Contactos | Gestión Contactos</h1>
<ol class="breadcrumb">
...
<li><a href="#">Listado Contactos</a></li>
</ol>
</div>
<div class="row">
<div class="col-md-12">
...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Contactos registrados</span></div>
<div class="actions">
<div class="btn-group">
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_contacto")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td> <%=ob_rs("nombre")%> </td>
<td>
<%Sql2= "SELECT apellidos, nombre from admins where id_admin ="&ob_rs("id_admin")&"" 
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
Ob_RS2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then%>
<%=ob_rs2("apellidos")%>, &nbsp;<%=ob_rs2("nombre")%>
<%end if
ob_rs2.close%>
</td>
<td>
<%Sql2= "SELECT titulo_esp, id_item from items where id_item ="&ob_rs("id_item")&""     '[RECOGEMOS DATOS SI MODIFICACION]
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
...
<%end if
ob_rs2.close%>
</td><%'fecha_now=FormatDateTime(cdate("12/25/1000"),2)
fecha_now=year(fecha_now)&"/"&completa0(month(fecha_now))&"/"&completa0(day(fecha_now))%>
<td data-order="<%=fecha_now%>"><%if ob_rs("fecha_in")<>"01/01/1000" then%><%=ob_rs("fecha_in")%><%end if%></td>
<%'fecha_now2=FormatDateTime(cdate(ob_rs("fecha_in")),2)
fecha_now2=year(fecha_now2)&"/"&completa0(month(fecha_now2))&"/"&completa0(day(fecha_now2))%>
<td data-order="<%=fecha_now2%>"><%if ob_rs("fecha_nxt")<>"01/01/1000" then%><%=ob_rs("fecha_nxt")%><%end if%></td>
<td><%
estado=ob_rs("estado")
Select case estado:
case "seguimiento"
esta="En seguimiento"
case "proceso"
esta="En proceso de venta"
case "finzalizado"
esta="Seguimiento finalizado"
case "cliente"
esta="Conversión a cliente"
case "formulario"
esta="Contacto de formulario"
case else
esta=""
end select%>
<%=esta%>
...
<ul class="dropdown-menu pull-right" id="acciones" >
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_contacto")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_contacto"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_contacto")&"&accion=vis"))%>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_contacto")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_contacto")%>&accion=del';}</script>
