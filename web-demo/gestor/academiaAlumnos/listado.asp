...
<h1>Academia | Gestión Alumnos <%if n="academia" then%>Academia<%else%>Summer School<%end if%> | <%if anyo_act<>1000 then%><%=anyo_act-1%>-<%=anyo_act%><%else%> Todos los años<%end if%></h1>
<ol class="breadcrumb">
...
anyo_ref=year(date)-4
if anyo_act=1000 then anyo_ah=anyo_ref else anyo_ah=year(date)-1 end if
if ndos<>"summer" then
Sql= "SELECT nombre, apellidos, academia_alumnos.grupo, fecha_alta, academia_alumnos.id_alumno from academia_alumnos, academia_clases where academia_alumnos.grupo<>'SUMMER_SCHOOL' AND academia_clases.id_alumno = academia_alumnos.id_alumno  AND year(fecha_alta) >= "&anyo_ah&""  
else
Sql= "SELECT nombre, apellidos, academia_alumnos.grupo, fecha_alta, academia_alumnos.id_alumno from academia_alumnos, academia_clases where academia_alumnos.grupo='SUMMER_SCHOOL' AND academia_clases.id_alumno = academia_alumnos.id_alumno  AND year(fecha_alta) >= "&anyo_ah&""  
end if 
...
<td> <%=limpia_clases(ob_rs("grupo"))%> </td>
<td>
