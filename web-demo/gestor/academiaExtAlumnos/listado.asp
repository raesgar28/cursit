...
if anyo_act="" or anyo_act =0 then anyo_act=year(date)end if%>
...
<h1>Extraescolares | Gestión Alumnos  | <%if anyo_act<>1000 then%><%=anyo_act-1%>-<%=anyo_act%><%else%> Todos los años<%end if%></h1>
<ol class="breadcrumb">
...
<li><a href="#">Listado Alumnos</a></li>
</ol>
</div>
<div class="row">
<div class="col-md-12">
...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Alumnos registrados</span></div>
<div class="actions">
<div class="btn-group">
...
<div class="btn-group"><a class="btn sbold green" href="javascript:;" data-toggle="dropdown"><i class="fa fa-wrench" aria-hidden="true"></i>&nbsp;Acciones</a>
...
<%if anyo_act=1000 then anyo_ah=2000 else anyo_ah=year(date)-1 end if

Sql= "SELECT nombre, apellidos, curso, extra_programas.id_alumno, id_actividad, fecha_alta, extra_alumnos.grupo, extra_programas.grupo from extra_alumnos, extra_programas WHERE extra_programas.id_alumno=extra_alumnos.id_alumno AND year(fecha_alta) >= "&anyo_ah&""  
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_alumno")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td> <%=ob_rs("apellidos")%>, <%=ob_rs("nombre")%> </td>
<td> <%=limpia_clases(ob_rs("curso"))%> </td>
<td>
<%Sql3= "SELECT nombre from extra_actividades where id_actividad="&ob_rs("id_actividad")&""  
Set Ob_RS3 = Server.CreateObject("ADODB.Recordset")
ob_rs3.Open Sql3, ob_connc,adOpenStatic, adCmdText
if not ob_rs3.eof then%>
<%=limpia_clases(ob_rs3("nombre"))%>&nbsp;<%=ob_rs("extra_programas.grupo")%>
<%end if
ob_rs3.close%>
</td>
<td> <%if ob_rs("fecha_alta")<>"" then%> <%=FormatDateTime(cdate(ob_rs("fecha_alta")),2)%> <%end if%></td>
<td>
...
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_alumno")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_alumno"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Gestionar</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_alumno")&"&accion=vis"))%>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_alumno")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_alumno")%>&accion=del';}</script>
...
var iStartDateCol = 4;                              // INDICA COLUMNA PARA BUSCAR FECHA
var iEndDateCol = 4;                               // INDICA COLUMNA PARA BUSCAR FECHA
...
var min = parseInt( $('#min').val(), 7 );
var max = parseInt( $('#max').val(), 7 );
