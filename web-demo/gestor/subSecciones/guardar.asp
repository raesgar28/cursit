...
fecha_in=now
fecha_mod=now 
orden_sub=request("orden_sub")
If IsNumeric(orden_sub) Then orden_sub=orden_sub else orden_sub=0 end if                                                     '[FIN REQUEST]

if op="up" then
Sql= "SELECT noticias.url_esp,generales.url_esp,items.url_esp,itemsSub.url_esp,categorias.url_esp,secciones.url_esp,subsecciones.url_esp,destinos.url_esp,ciudades.url_esp from noticias,generales,items,itemsSub,categorias,secciones,subsecciones,destinos,ciudades WHERE (subsecciones.url_esp ='"&url_esp&"' and id_subseccion <> "&cod&") or (generales.url_esp = '"&url_esp&"' or items.url_esp= '"&url_esp&"' or itemsSub.url_esp = '"&url_esp&"' or categorias.url_esp = '"&url_esp&"' or secciones.url_esp= '"&url_esp&"' or noticias.url_esp = '"&url_esp&"' or destinos.url_esp = '"&url_esp&"' or ciudades.url_esp = '"&url_esp&"' )"   
else
...
mySQL="INSERT INTO subsecciones (titulo_esp,texto_esp,url_esp,meta_title,meta_key,meta_desc,visible,fecha_in,disponible,destacado,orden)" 				
	...
	mySQL=mySQL & "" & orden_sub & ""
	mySQL=mySQL & ");"
set ob_rs=Ob_Conn.execute(mySQL)
set ob_rs = Ob_Conn.execute("select MAX(id_subseccion) from subsecciones") 
cod = ob_rs(0)
ob_rs.close
if id_seccion<>""then                                              '[RELACIONAMOS EN REL ID_WEB <> ID_CATEGORIA]
cadena_rel = split(id_seccion, ",")
for i=0 to ubound(cadena_rel)
Sql3_2= "SELECT id_categoria, id_seccion from rel where id_seccion = "&cadena_rel(i)&" "   
...
ob_rs3_2.close
if id_categoria=""then id_categoria=0
mySQL="INSERT INTO rel (id_seccion,id_categoria,id_subseccion)" 				
	mySQL=mySQL & "VALUES ("         '[INSERTAMOS]
...
mySQL=mySQL & "orden = " & orden_sub & ""
mySQL=mySQL & " WHERE id_subseccion = " & cod & "" 
set ob_rs=Ob_Conn.execute(mySQL)
if id_seccion<>""then                                              '[RELACIONAMOS EN REL ID SECCION ID CATEGROIA]
mySQL="DELETE FROM rel where id_subseccion =" & cod & " and id_seccion <> 0" 'ELIMINAMOS PRIMERO
set ob_rs=Ob_conn.execute(mySQL)
cadena_rel = split(id_seccion, ",")
for i=0 to ubound(cadena_rel)
Sql3_2= "SELECT id_categoria, id_seccion from rel where id_seccion = "&cadena_rel(i)&" group by id_categoria, id_seccion "   
Set ob_rs3_2 = Server.CreateObject("ADODB.Recordset")
ob_rs3_2.Open Sql3_2, ob_conn,adOpenStatic, adCmdText
if not ob_rs3_2.eof then
...
id_categoria=ob_rs3_2("id_categoria")
if id_categoria=""then id_categoria=0
mySQL="INSERT INTO rel (id_seccion,id_categoria,id_subseccion)" 	'id_categoria,			
	mySQL=mySQL & "VALUES ("         '[INSERTAMOS]
	mySQL=mySQL & "'" & cadena_rel(i) & "',"
	mySQL=mySQL & "'" & id_categoria & "',"
	mySQL=mySQL & "'" & cod & "'"
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_subseccion, archivos.id_archivo, archivos.campo from rel, archivos where rel.id_subseccion="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and archivos.campo ='"&campo&"'"                    
Ob_Command.CommandText = Sql
...
id_archivo = ob_rs(0)
tipo_rel=tipo
mySQL="INSERT INTO rel (id_subseccion,id_archivo,tipo_rel,fecha_in,fecha_mod)" '[RELACIONAMOS IMAGEN CON WEB]
	mySQL=mySQL & "VALUES ("
	mySQL=mySQL & "" & cod & ","
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_subSeccion, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_subSeccion="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo "                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
do while not ob_rs3.eof
...
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_subSeccion, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_subSeccion="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and (archivos.campo <> 'archivo_1' and archivos.campo <> 'archivo_2' and archivos.campo <> 'archivo_17' and archivos.campo <> 'archivo_18' and archivos.campo <> 'archivo_19' and archivos.campo <> 'archivo_20') order by archivos.orden"                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
ord=3
...
Sql= "SELECT id_subseccion, url_esp from subsecciones"                            '[COMENZAMOS REESCRITURA]
Ob_Command.CommandText = Sql
SET Ob_RS = Ob_Command.Execute ()
if not ob_rs.eof then
set fso = createobject("scripting.filesystemobject")
Set act = fso.CreateTextFile(server.mappath(rd&"mapas_url/subsecciones.txt"), true)
do while not ob_rs.eof
url = ob_rs("url_esp")
act.WriteLine ""&limpia_url(url)&" "&ob_rs("id_subseccion")&"" 
ob_rs.movenext
loop
act.Close
end if
