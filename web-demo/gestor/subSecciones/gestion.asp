...
<%if cod<>""then
Sql= "SELECT id_subseccion, id_archivo from rel where id_subseccion="&cod&" and id_archivo<>0" '[IMAGENES VIDEOS TOTALES]
...
ob_rs.close
Sql= "SELECT titulo_esp,texto_esp,url_esp,meta_title,meta_key,meta_desc,visible,disponible,destacado,fecha_in, orden from subsecciones where id_subseccion="&cod&""     '[RECOGEMOS DATOS SI MODIFICACION]
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
...
orden_sub=ob_rs("orden")
end if
ob_rs.close
Sql= "SELECT id_subseccion,id_archivo from rel where id_subseccion="&cod&""
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
i=1
...
<h1>Gestión SubSecciones Cursos | <%if cod<>""then%>Modificar<%else%>Crear<%end if%> </h1>
<ol class="breadcrumb">
...
<a href="<%=r%>subsecciones/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>">Listado SubSecciones Cursos</a></li>
<li class="active">Gestión SubSeciones Cursos</li>
...
<li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat))%><a href="<%=r%>subsecciones/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>"><i class="icon-list"></i> Listado subSecciones<%if num_ss<>""then%><span class="baddge badge badge-default"> <%=num_ss%> </span><%else%><span class="baddge badge badge-warning"> 0 </span><%end if%></a></li>
<li class="active"><a href="#"><i class="icon-pin"></i> Gestión subSecciones</a></li>
<li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n=ac&ndos=css&ntres=s&ncuat="))%><a href="<%=r%>secciones/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n=ac&ndos=css&ntres=s&ncuat=")%>"><i class="icon-note"></i> Secciones Cursos <%if num_s<>""then%><span class="baddge badge badge-default"> <%=num_s%> </span><%else%><span class="baddge badge badge-warning"> 0 </span><%end if%></a></li>
...
<li><a href="javascript:modal_programas();"><i class="icon-share-alt"></i> Listar Programas <%if num_it<>""then%><span class="baddge badge badge-default"> <%=num_it%> </span><%else%><span class="baddge badge badge-warning"> 0 </span><%end if%></a></li></ul>
</nav>
</div><%end if%>
<div class="page-content-col">
<div class="portlet light portlet-fit portlet-form bordered">
<div class="portlet-title tabbable-line">
<div class="caption"><i class="icon-note font-green"></i><span class="caption-subject font-green sbold uppercase">Gestión subSección Curso  <%if titulo_esp<>""then%>| <%=titulo_esp%><%end if%></span></div>
...
<label class="col-md-4 control-label" for="titulo_esp" >Nombre subSección.<span class="required">*</span></label>
<div class="col-md-8">
...
<!--FIN IMAGEN-->
<div class="form-group form-md-line-input">
<label class="col-md-4 control-label" for="orden_sub" >Orden.</label>
<div class="col-md-3">
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="orden_sub" id="orden_sub" maxlength="160" value="<%if orden_sub<>0 and cod<>"" then%><%=orden_sub%><%else%><%if num_ss<>0then%><%=num_ss+1%><%else%>1<%end if%><%end if%>"><div class="form-control-focus"> </div><i class="fa fa-sort-numeric-desc" aria-hidden="true"></i><span class="help-block">orden subsección</span></div>
</div>
</div>
...
if cod<>""then
Sql2= "SELECT id_subseccion, id_seccion,id_categoria from rel where id_subseccion = "&cod&" and id_seccion= "&id_seccion&" "   '[ASOCIAMOS CON UNA SEC]
Ob_Command.CommandText = Sql2
SET Ob_RS2 = Ob_Command.Execute ()
if not ob_rs2.eof then
sel="si"
...
<div class="form-control-focus"> </div><i class="fa fa-globe" aria-hidden="true"></i><span class="help-block">selecciona una sección</span></div>
</div>
</div>
<%else%>
<script>
...
verticalEdge:'top'},
n=$(this);""!=$.trim('No se puede crear ninguna subsección')&&(t.heading=$.trim('No se puede crear ninguna subsección')),
t.sticky||(t.life='12000'),
$.notific8("zindex",11500),
$.notific8($.trim('Por favor, crea una Sección en Gestión Secciones'),t),
...
<a data-toggle="modal" data-target="#secciones_mod" class="f_red"><i class="icon-note"></i> Debes crear una sección para poder continuar</a> </div>
</div>
</div>
<%end if
ob_rs.close
...
<button type="button" class="btn green <%if sel_sec<>"si" then%>disabled<%end if%>" <%if sel_sec="si" then%>onclick="Enviar_subsecciones(this.form);"<%end if%> value="enviar datos" language="javascript"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> modificar datos</button>
...
<script>
CKEDITOR.replace('texto_esp',{filebrowserBrowseUrl: '/gestor/ckfinder/CKFinderScripts/ckfinder.html',filebrowserUploadUrl: '/gestor/ckfinder/connector?command=QuickUpload&type=Files',toolbar : 'Medida'});$(document).ready(function() {
