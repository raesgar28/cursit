...
<%if cod<>"" then
Sql= "SELECT * from rel where id_noticia="&cod&" and id_archivo<>0" '[IMAGENES VIDEOS TOTALES]
...
Sql= "SELECT * from noticias where id_noticia="&cod&""     '[RECOGEMOS DATOS SI MODIFICACION]
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
...
ob_rs.close
Sql= "SELECT * from rel where id_noticia="&cod&""
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
i=1
...
<h1>Gestión Noticias |  <%if cod<>""then%>Modificar<%else%>Crear<%end if%></h1>
<ol class="breadcrumb">
...
<a href="<%=r%>noticias/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>">Listado Noticias</a></li>
<li class="active">Gestión Noticia</li>
...
<li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat))%>
<a href="<%=r%>noticias/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>"><i class="icon-list"></i> Listado Noticias
<%if num_nt<>""then%><span class="baddge badge badge-default"> <%=num_nt%> </span><%else%><span class="baddge badge badge-warning"> 0 </span><%end if%></a></li>
<li class="active"><a href="#"><i class="icon-pin"></i> Gestión Noticia</a></li>
</ul>
</nav>
</div><%end if%>
...
<div class="caption"><i class="icon-note font-green"></i><span class="caption-subject font-green sbold uppercase">Gestión Noticia <%if titulo_esp<>""then%>| <%=titulo_esp%><%end if%></span></div>
...
<label class="col-md-4 control-label" for="titulo_esp" >Título noticia.<span class="required">*</span></label>
<div class="col-md-8">
...
<i class="fa fa-newspaper-o" aria-hidden="true"></i><span class="help-block">debes introducir un título</span></div></div></div>
<!--IMAGEN-->
...
<div class="col-md-3">
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="orden_general" id="orden_general" maxlength="160" value="<%if orden_general<>0 and cod<>"" then%><%=orden_general%><%else%><%=num_pg+1%><%end if%>"><div class="form-control-focus"></div><i class="fa fa-sort-numeric-desc" aria-hidden="true"></i><span class="help-block">orden del curso</span></div>
</div>
</div>


<div class="form-group form-md-line-input">
<label class="col-md-3 control-label" for="fecha_in" >Fecha.</label>
<div class="col-md-5" ><div class="input-icon">
<div class="input-group date date-picker" data-date-format="dd/mm/yyyy" ><input type="text" class="form-control form-filter input-sm" id="fecha_in" name="fecha_in" <%if cod<>"" and fecha_mod<>"01/01/1000"then%>value="<%=fecha_mod%>"<%else%><%=date%><%end if%>><span class="input-group-btn"><button class="btn btn-sm default" type="button"><i class="fa fa-calendar"></i></button></span></div>
...
<div class="col-md-12">
<div class="form-group form-md-line-input">
<label class="col-md-2 control-label margin-left-12" for="texto_esp">Resumen.</label>
<div class="col-md-8"><textarea class="form-control" name="texto_esp" id="texto_esp" rows="3"><%=texto_esp%></textarea></div>
</div>
</div><!--col-md-12-->
<div class="col-md-12">
<div class="form-group form-md-line-input">
<label class="col-md-2 control-label margin-left-12" for="texto_esp2">Texto.</label>
...
<%if edi="" then%><a href="<%=r%>noticias/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>" class="btn default"><i class="fa fa-reply" aria-hidden="true"></i> volver al listado</a><%end if%>
</div>
</div>
</div>
