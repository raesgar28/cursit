...
ob_rs.close
Sql= "SELECT titulo_esp,id_item from items where id_item="&cod_prog&" " 
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
...
<h1>Programas | <%=tit_categoria%> | Gestión Cursos | <%=tit_programa%></h1>
<ol class="breadcrumb">
...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Cursos registrados</span></div>
<div class="actions">
<div class="btn-group">
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&edi="&edi&"&cod_prog="&cod_prog&"&cod_cat="&cod_cat))%>
<a href="gestion.jpk2?enc=<%=EnCrypt("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&edi="&edi&"&cod_prog="&cod_prog)&"&cod_cat="&cod_cat%>" class="btn sbold red">Añadir nuevo&nbsp;<i class="fa fa-plus"></i></a>
</div>
<div class="btn-group">
<%if edi=""then%>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&cod_prog&"&cod_cat="&cod_cat))%>
<a href="<%=r%>items/gestion.jpk2?enc=<%=EnCrypt("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&cod_prog&"&cod_cat="&cod_cat)%>" class="btn sbold green">Volver al programa&nbsp;<i class="fa fa-undo" aria-hidden="true"></i></a><%end if%>
</div>
...
<%Sql= "SELECT rel.id_item, rel.id_itemSub, itemsSub.id_itemSub, itemsSub.titulo_esp, itemsSub.fecha_in, itemsSub.visible, itemsSub.disponible from itemsSub, rel where rel.id_item="&cod_prog&" and rel.id_itemSub=itemsSub.id_itemSub"   
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
ob_rs.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
i=1%>
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("itemsSub.id_itemSub")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
...
<ul class="dropdown-menu pull-right" id="acciones" >
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("itemsSub.id_itemSub")&"&cod_cat="&cod_cat&"&edi="&edi&"&cod_prog="&cod_prog&"&edi="&edi))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("itemsSub.id_itemSub")&"&cod_cat="&cod_cat&"&edi="&edi&"&cod_prog="&cod_prog&"&edi="&edi)%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>

<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=vis&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=vis&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi)%>&" ><i class="fa fa-eye" aria-hidden="true"></i> Visible</a></li>


<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=vis_no&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=vis_no&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi)%>&"  ><i class="fa fa-eye-slash" aria-hidden="true"></i> No visible</a></li>



<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=dis&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=dis&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi)%>&" ><i class="fa fa-check" aria-hidden="true"></i> Disponible</a></li>

<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=dis_no&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("itemsSub.id_itemSub")&"&accion=dis_no&cod_cat="&cod_cat&"&cod_prog="&cod_prog&"&edi="&edi)%>&"  ><i class="fa fa-times" aria-hidden="true"></i> No disponible</a></li>



<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("itemsSub.id_itemSub")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("itemsSub.id_itemSub")%>&accion=del&cod_cat=<%=cod_cat%>&cod_prog=<%=cod_prog%>&edi=<%=edi%>';}</script>
...
<%if edi="rap" then%>parent.sav_gest_cursos_off();//BOTON GUARDAR CURSOS DESDE MODAL<%end if%>
