...
<h1>Categorías | Gestión Categorías</h1>
<ol class="breadcrumb">
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_categoria")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td> <%=ob_rs("titulo_esp")%> </td>
<td> <%=ob_rs("titulo_crm_esp")%> </td>
<td>
<%Sql2= "SELECT * from rel where id_categoria = "&ob_rs("id_categoria")&" and id_web <> 0 "   
Set ob_rs2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_conn,adOpenStatic, adCmdText
if not ob_rs2.eof then
do while not ob_rs2.eof
id_web=ob_rs2("id_web")
if id_web<>""then
Sql3= "SELECT * from webs where id_web = "&id_web&" "   
Set ob_rs3 = Server.CreateObject("ADODB.Recordset")
ob_rs3.Open Sql3, ob_conn,adOpenStatic, adCmdText
if not ob_rs3.eof then
Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n=ad&ndos=con&ntres=w&ncuat="&ncuat&"&cod="&id_web))%>
<a href="<%=r%>webs/gestion.jpk2?enc=<%=EnCrypt("?r=" & r & "&rd=" & rd &"&n=ad&ndos=con&ntres=w&ncuat="&ncuat&"&cod="&id_web)%>" class="enlace_tabla_in"><%=ob_rs3("titulo_esp")%><i class="fa fa-pencil-square-o margin-left-12" style="color:#ddd" aria-hidden="true"></i></a>
<%end if
ob_rs3.close
end if
ob_rs2.movenext%><br />
...
<ul class="dropdown-menu pull-right" id="acciones" >
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_categoria")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_categoria"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=vis"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=vis")%>&" ><i class="fa fa-eye" aria-hidden="true"></i> Visible</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=vis_no"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=vis_no")%>&"  ><i class="fa fa-eye-slash" aria-hidden="true"></i> No visible</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=des"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=des")%>&" ><i class="fa fa-star" aria-hidden="true"></i> Destacado</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=des_no"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=des_no")%>&"  ><i class="fa fa-star-half-o" aria-hidden="true"></i> No destacado</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=dis"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=dis")%>&" ><i class="fa fa-check" aria-hidden="true"></i> Disponible</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=dis_no"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_categoria")&"&accion=dis_no")%>&"  ><i class="fa fa-times" aria-hidden="true"></i> No disponible</a></li>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_categoria")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_categoria")%>&accion=del';}</script>
...
{orderable: false, targets:   8},//ULTIMA -> ACCIONES NO SE PUEDE ORDENAR
