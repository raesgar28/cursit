...
Sql= "SELECT url_esp FROM ciudades WHERE url_esp ='"&url_esp&"' and id_ciudad <> "&cod&""   
else
Sql= "SELECT url_esp FROM ciudades WHERE url_esp ='"&url_esp&"'"   
end if
...
mySQL="INSERT INTO ciudades (titulo_esp,texto_esp,url_esp,meta_title,meta_key,meta_desc,visible,fecha_in,disponible,destacado)" 				
...
set ob_rs = Ob_Conn.execute("select MAX(id_ciudad) from ciudades") 
cod = ob_rs(0)
ob_rs.close
end if                                                            '[FIN OPCION INSERTAR]
if op="up"then                                                    '[OPCION MODIFICAR]
mySQL="UPDATE ciudades SET "
...
mySQL=mySQL & " WHERE id_ciudad = " & cod & "" 
set ob_rs=Ob_Conn.execute(mySQL)
end if                                                           '[FIN OPCION MODIFICAR]
if id_destino<>""then                                              '[RELACIONAMOS EN REL ID_DESTINO]
cadena_rel = split(id_destino, ",")
for i=0 to ubound(cadena_rel)
mySQL="INSERT INTO rel (id_destino,id_ciudad)" 				
	mySQL=mySQL & "VALUES ("         '[INSERTAMOS]
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_ciudad, archivos.id_archivo, archivos.campo from rel, archivos where rel.id_ciudad="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and archivos.campo ='"&campo&"'"                    
Ob_Command.CommandText = Sql
...
id_archivo = ob_rs(0)
tipo_rel=tipo
mySQL="INSERT INTO rel (id_ciudad,id_archivo,tipo_rel,fecha_in,fecha_mod)" '[RELACIONAMOS IMAGEN CON WEB]
	mySQL=mySQL & "VALUES ("
	mySQL=mySQL & "" & cod & ","
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_ciudad, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_ciudad="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo "                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
do while not ob_rs3.eof
...
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_ciudad, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_ciudad="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and (archivos.campo <> 'archivo_1' and archivos.campo <> 'archivo_2' and archivos.campo <> 'archivo_17' and archivos.campo <> 'archivo_18' and archivos.campo <> 'archivo_19' and archivos.campo <> 'archivo_20') order by archivos.orden"                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
ord=3
...
Sql= "SELECT id_ciudad, url_esp from ciudades"                            '[COMENZAMOS REESCRITURA]
Ob_Command.CommandText = Sql
SET Ob_RS = Ob_Command.Execute ()
if not ob_rs.eof then
set fso = createobject("scripting.filesystemobject")
Set act = fso.CreateTextFile(server.mappath(rd&"mapas_url/ciudades.txt"), true)
do while not ob_rs.eof
url = ob_rs("url_esp")
act.WriteLine ""&limpia_url(url)&" "&ob_rs("id_ciudad")&"" 
ob_rs.movenext
loop
act.Close
end if
