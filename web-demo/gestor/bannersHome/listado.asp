...
<h1>Banners Home | Gestión Banners</h1>
<ol class="breadcrumb">
...
<li><a href="#">Listado Banners</a></li>
</ol>
</div>
<div class="row">
<div class="col-md-12">
...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Banners registrados</span></div>
<div class="actions">
<div class="btn-group">
...
<%Sql= "SELECT * from generales WHERE tipo='bh'"   
...
<th style="text-align:center">Tipo & Orden</th>
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_general")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td> <%if ob_rs("tipo")="hm"then%> Home<%else%><%=ob_rs("titulo_esp")%><%end if%> </td>
<td style="text-align:center"><%if ob_rs("visible")="si"then%><i class="fa fa-check f_green" aria-hidden="true"></i><span class="f_white_p">Sí</span><%else%><i class="fa fa-times f_red" aria-hidden="true"></i><span class="f_white_p">No</span><%end if%></td>
<td style="text-align:center">
<%Sql2= "SELECT rel.id_archivo, id_general, archivos.orden FROM rel, archivos WHERE id_general="&ob_rs("id_general")&" AND rel.id_archivo=archivos.id_archivo AND archivos.tipo='fil' "   
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_conn,adOpenStatic, adCmdText
if not ob_rs2.eof then
video="ok"%><i class="fa fa-video-camera" aria-hidden="true" style="color:#999;font-size:1.5em"></i>&nbsp;(<%=ob_rs("orden")%>)
<%end if
ob_rs2.close
if video<>"ok" then
Sql2= "SELECT rel.id_archivo, id_general, archivos.orden FROM rel, archivos WHERE id_general="&ob_rs("id_general")&" AND rel.id_archivo=archivos.id_archivo AND tipo='img' "   
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_conn,adOpenStatic, adCmdText
if not ob_rs2.eof then
imagen="ok"%>
<i class="fa fa-picture-o" aria-hidden="true" style="color:#999;font-size:1.5em"></i>&nbsp;(<%=ob_rs("orden")%>)
<%end if
ob_rs2.close
end if
if video<>"ok" and imagen<>"ok"then%>
<i class="fa fa-times" aria-hidden="true" style="color:#999;font-size:1.5em"></i>
<%end if
video=""
imagen=""%>
</td>
...
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ob_rs("tipo")&"&ncuat="&ncuat&"&cod="&ob_rs("id_general")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ob_rs("tipo")&"&ncuat="&ncuat&"&cod="&ob_rs("id_general"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_general")&"&accion=vis"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_general")&"&accion=vis")%>&" ><i class="fa fa-eye" aria-hidden="true"></i> Visible</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_general")&"&accion=vis_no"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_general")&"&accion=vis_no")%>&"  ><i class="fa fa-eye-slash" aria-hidden="true"></i> No visible</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_general")&"&accion=des"))%>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_general")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_general")%>&accion=del';}</script>
