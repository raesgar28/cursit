...
tip=request("tip")
if tip ="" then tip = request_equerystring("tip") end if
...
Sql= "SELECT id_agente, id_usuario, nom_categoria, id_categoria,suma_total_p,cod_descuento,id_agente_cod, completado, mail_aviso, a_observaciones_crm, cancela, fecha_cancela from programas where id_programa="&cod_prog&" order by id_programa desc"     '[DATOS AGENTE]
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_connb,adOpenStatic, adCmdText
if not ob_rs.eof then
...
a_observaciones_crm=ob_rs("a_observaciones_crm")
...
tab1_5="active"
case 13:
ms_t="Programa cancelado con éxito."
tab1="active"
tab1_1="active"
case 14:
ms_t="Programa activado con éxito."
tab1="active"
tab1_1="active"
case else
...
<div class="portlet-title tabbable-line"><div class="caption"><i class="icon-note font-green"></i><span class="caption-subject font-green sbold uppercase">Gestión Alumno &nbsp;|&nbsp;<span style="color:#999"><%=a_nombre%>&nbsp;<%=a_apellidos%>&nbsp;<%if tip<>"eli"then%>|&nbsp;<i class="fa fa-arrow-right" aria-hidden="true" style="margin-right:8px"></i><%=nom_item%>&nbsp;·&nbsp;<%=nom_curso%><%end if%></span></span> <%if cancela="si" then%><span class="label label-danger c-font-slim" style="margin-left:15px">cancelado [<%=fecha_cancela%>]</span> <%end if%> </div>
<ul class="nav nav-tabs"><%if tip<>"eli" then%><li class="<%=tab1%>"><a href="#tab_1" data-toggle="tab"> Programa </a></li><%end if%>
<%if tip<>"eli" then%><li class="<%=tab2%>"><a href="#tab_2" data-toggle="tab"> Datos Generales <%if num_img_tot<>""then%><span class="badge badge-success" style="margin-left:5px""> <%=num_img_tot%> </span><%end if%></a></li><%end if%>
...
<%if tip<>"eli" then%><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod_prog="&cod_prog&"&accion=env_mail&ver="&ver&"&cod_cat="&cod_cat&"&check="&cod_prog))%>
...
<li class="divider"></li>
<%if cancela="si" then%>
<li><a href="javascript:descancelarPrograma();"><i class="fa fa-circle" aria-hidden="true"></i> Recuperar programa cancelado</a></li>
<%else%><li><a href="javascript:cancelarPrograma();"><i class="fa fa-ban" aria-hidden="true"></i> Cancelar programa</a></li>
<%end if%>

<li class="divider"></li>
<li><a href="javascript:eliminarPrograma();"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar programa</a></li><%end if%>
...
<div class="alert-warning" style="padding:17px"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Factura no generada. El programa <a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs3("id_alumno")&"&ver="&ver&"&cod_cat="&cod_cat&"&cod_prog="&ob_rs3("id_programa"))%>">[<%=ob_rs3("id_programa")%>]</a> ya tiene generada la factura [<%=n_fac_w%>]</div>
<%end if
ob_rs3.close
end if
ob_rs2.close
...
<!--tab_Sup_1 PROGRAMAS-->
<%if tip<>"eli" then%><div class="tab-pane <%=tab1%>" id="tab_1">
<div class="form-body">
<div class="row">
<div class="col-md-12">
...
if pr_medico_pr=""then pr_medico_pr_c=0 else pr_medico_pr_c=pr_medico_pr end if 'CHECK DESCUADRES
if precio_sup=""then precio_sup_c=0 else precio_sup_c=precio_sup end if
if pr_medico_pr=""then pr_medico_pr_c=0 else pr_medico_pr_c=pr_medico_pr end if
if pr_cancelacion_pr=""then pr_cancelacion_pr_c=0 else pr_cancelacion_pr_c=pr_cancelacion_pr end if
if tasas_aerop_pr=""then tasas_aerop_pr_c=0 else tasas_aerop_pr_c=tasas_aerop_pr end if
if pr_examen_pr=""then pr_examen_pr_c=0 else pr_examen_pr_c=pr_examen_pr end if
if pr_gestion_bill_pr=""then pr_gestion_bill_pr_c=0 else pr_gestion_bill_pr_c=pr_gestion_bill_pr end if
if precio_alo=""then precio_alo_c=0 else precio_alo_c=precio_alo end if
if tasas_opciones=""then tasas_opciones_c=0 else tasas_opciones_c=tasas_opciones end if
if precio_sup_alo=""then precio_sup_alo_c=0 else precio_sup_alo_c=precio_sup_alo end if
if tasas_opciones=""then tasas_opciones_c=0 else tasas_opciones_c=tasas_opciones end if
if precio_sup=""then precio_sup_c=0 else precio_sup_c=precio_sup end if
...
opciones_tot=0
Sql= "SELECT opcion_pre_curso, opcion_pre_prog FROM programasOp WHERE id_programa="&cod_prog&" and id_alumno="&id_alumno&" and (id_itemsubop<>0 or id_itemOp <>0)"  
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_connb,adOpenStatic, adCmdText
if not ob_rs.eof then
do while not ob_rs.eof
opci_pre_curso=ob_rs("opcion_pre_curso") 
opci_pre_prog=ob_rs("opcion_pre_prog") 
if opci_pre_curso="" or ISNULL(opci_pre_curso) then opci_pre_curso=0 end if
if opci_pre_prog="" or ISNULL(opci_pre_prog) then opci_pre_prog=0 end if
opciones_tot=opciones_tot+opci_pre_curso+opci_pre_prog
opci_pre_curso=""
poci_pro_prog=""
ob_rs.movenext
loop
...
opciones_tot2=0
Sql= "SELECT precio FROM programasOpAdd WHERE id_programa="&cod_prog&" "  
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_connb,adOpenStatic, adCmdText
if not ob_rs.eof then
do while not ob_rs.eof
opciones_tot2=opciones_tot2+ob_rs("precio")
ob_rs.movenext
loop
end if
ob_rs.close
...
tot_anticipos=0
Sql= "SELECT precio FROM programasAnticipos WHERE id_programa="&cod_prog&" and pagado='si'"  
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_connb,adOpenStatic, adCmdText
if not ob_rs.eof then
do while not ob_rs.eof
tot_anticipos=tot_anticipos+ob_rs("precio")
tot_anticipos=FormatNumber(ccur(tot_anticipos),2)
ob_rs.movenext
loop
end if
ob_rs.close
...
falta_pagar_n=suma_total-tot_anticipos
if falta_pagar_n<>""then falta_pagar_n=FormatNumber(ccur(falta_pagar_n))end if

if estadoPago="Aceptada" then
'falta_pagar_n=falta_pagar_n-precioReserva-pr_cancelacion_pr_c
falta_pagar_n=FormatNumber(ccur(CDbl(falta_pagar_n)-CDbl(precioReserva)-CDbl(pr_cancelacion_pr_c)),2)
end if
if falta_pagar_n<>""then falta_pagar_n=FormatNumber(ccur(falta_pagar_n))end if
if precio_cur="" then precio_cur=0 end if
if pr_medico_pr_c="" then pr_medico_pr_c=0 end if
if precio_sup_c="" then precio_sup_c=0 end if
if pr_cancelacion_pr_c="" then pr_cancelacion_pr_c=0 end if
if tasas_aerop_pr_c="" then tasas_aerop_pr_c=0 end if
if pr_examen_pr_c="" then pr_examen_pr_c=0 end if
if precio_alo_c="" then precio_alo_c=0 end if
if tasas_opciones_c="" then tasas_opciones_c=0 end if
if precio_sup_alo_c="" then precio_sup_alo_c=0 end if
if opciones_tot="" or ISNULL(opciones_tot) then opciones_tot=0 end if
if opciones_tot2="" or ISNULL(opciones_tot2) then opciones_tot2=0 end if
if precio_cur_a="" or ISNULL(precio_cur_a) then precio_cur_a=0 end if
total_check=FormatNumber(ccur(precio_cur_a+CDbl(pr_medico_pr_c)+CDbl(precio_sup_c)+CDbl(pr_cancelacion_pr_c)+CDbl(tasas_aerop_pr_c)+CDbl(pr_examen_pr_c)+CDbl(precio_alo_c)+CDbl(opciones_tot)+CDbl(opciones_tot2)+CDbl(pr_gestion_bill_pr_c)),2)'+precio_sup_alo_c'tasas_opciones_c+



if falta_pagar_n<>falta_pagar or total_check<>suma_total then%>



<div class="alert-warning" style="padding:17px;margin-bottom:25px"><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Revisar posible descuadre en contabilidad de programa</div>
<%end if  'FIN CHECK DESCUADRES%>
...
<div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="f_fin" >Fecha de regreso.</label><div class="col-md-4"><div class="input-icon"><input type="text" class="form-control" placeholder="" name="f_fin" id="f_fin" maxlength="160" value="<%if f_fin<>"01/01/1000" and f_fin<>"08/01/1111" and year(f_fin)<>"1000" and year(f_fin)<>"1111" then%><%=f_fin%>
<%else
if year(f_inicio)<>"1000" and year(f_inicio)<>"1111" then%>
<%=f_inicio + semanas * 7%>
<%end if%>
<%end if%>"><div class="form-control-focus"> </div><i class="fa fa-calendar-times-o" aria-hidden="true"></i><span class="help-block">dd/mm/aaaa</span></div></div></div>
<!--SEMANAS-->
<%if id_itemSub<>"" then
...
<%Sql2= "SELECT nom_opcion_curso FROM programasOp WHERE id_programa="&cod_prog&" AND transfer='si' "
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
Ob_RS2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then%>
<div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="nom_curso">Traslado aeropuerto.</label><div class="col-md-8"><div class="input-icon"><input type="text" class="form-control" placeholder="" name="nom_curso" id="nom_curso" maxlength="160" value="<%=ob_rs2("nom_opcion_curso")%>" readonly><div class="form-control-focus"> </div><i class="fa fa-plane" aria-hidden="true"></i></div></div></div>
<%end if
ob_rs2.close%>
...
<div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="a_observaciones_curso_n" >Observaciones alumno.</label><div class="col-md-8"><div class="input-icon"><textarea name="a_observaciones_curso_n" class="form-control" id="a_observaciones_curso_n" placeholder=""><%=a_observaciones_curso%></textarea><div class="form-control-focus"></div><i class="fa fa-quote-right" aria-hidden="true"></i><span class="help-block">observaciones del alumno (modificar en contabilidad)</span></div></div></div>

<div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="tipo_cambio" >Tipo de cambio.</label><div class="col-md-5"><div class="input-icon"><input type="text" class="form-control" placeholder="" name="tipo_cambio" id="tipo_cambio" maxlength="160" value="<%=tipo_cambio%>" ><div class="form-control-focus"> </div><i class="fa fa-exchange" aria-hidden="true"></i></div></div></div>
...
<%do while not ob_rs.eof
'VEMOS SI LO HEMOS SELECCIONADO
Sql2= "SELECT opcion_pre_curso, nom_opcion_curso, id_programaOp, id_itemSubOp FROM programasOp WHERE id_programa ="&cod_prog&" AND trim(StrConv(nom_opcion_curso, 2, 1042))='"&ob_rs("opcion_esp")&"' and id_itemSubOp<>0"
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
Ob_RS2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then
op_curso_sel="si"
id_itemSubOp=ob_rs2("id_itemSubOp")
end if
ob_rs2.close%>
<div class="md-checkbox">
<input type="checkbox" id="opc[<%=i%>]" class="c-check" value="<%=ob_rs("id_itemSubOp")%>|<%=ob_rs("opcion_esp")%>" name="opc[<%=i%>]" autocomplete="off" <%if op_curso_sel="si"  then%>Checked<%end if%> class="md-check"><label for="opc[<%=i%>]"><span></span><span class="check"></span><span class="box"></span> <%=ob_rs("opcion_esp")%></label>
</div>
<%ob_rs.movenext
op_curso_sel=""
id_itemSubOp=""
...
'VEMOS SI HEMOS ELEGIDO OPCION Y LUEGO SE HA CAMBIADO EN PROGRAMA
Sql2= "SELECT opcion_pre_curso, nom_opcion_curso, id_programaOp,id_itemSubOp FROM programasOp WHERE id_programa ="&cod_prog&" AND nom_opcion_curso<>'' AND transfer IS NULL "
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
Ob_RS2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then
i=i+1
...
<div class="md-checkbox">
<input type="checkbox" id="opc[<%=i%>]" class="c-check" value="<%=ob_rs2("id_programaOp")%>|<%=ob_rs2("nom_opcion_curso")%>" name="opc[<%=i%>]" autocomplete="off" <%if ob_rs2("id_ItemSubOp")<>0 then%>Checked<%end if%> class="md-check">
...
Mail aviso.<%if mail_aviso<>0 and mail_aviso<>"" then%>(<%=mail_aviso%>)<%end if%></label></div></div></div></div></div></div></div>
...
<div class="col-md-12" >
<div class="note note-info magin-top-0" style="margin-bottom:30px;margin-top:30px"><h4 class="">Notas importantes</h4>
<p><i class="fa fa-info-circle" aria-hidden="true"></i> Usar comas (,) para decimales (ej:120,20) <br><i class="fa fa-info-circle" aria-hidden="true"></i> No usar puntos (.) para millares (ej: 2500)<br><i class="fa fa-info-circle" aria-hidden="true"></i> Revisar resultados manualmente y modificar en caso necesario.</p></div>
</div>
...
<%if alo_concepto<>""then%><div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="tasas_concepto" >Tasas <%=alo_concepto%>. </label><div class="col-md-8"><div class="input-icon"><input name="tasas_concepto" type="text" class="form-control" id="tasas_concepto" placeholder="" onKeyUp="suma();" value="<%=tasas_concepto%>" maxlength="160"><div class="form-control-focus"> </div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">tasas si las hubiera</span></div></div></div><%end if%><div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="precioReserva" >Reserva.</label><div class="col-md-8"><div class="input-icon"><input type="text" class="form-control" placeholder="" name="precioReserva" id="precioReserva" maxlength="160" value="<%=precioReserva%>" onKeyUp="suma();"><div class="form-control-focus"> </div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">precio de la reserva</span></div></div></div>
...
<%if alo_opcion<>""then%><div class="form-group form-md-line-input"><label class="col-md-3 control-label" for="tasas_opciones" >Tasas <%=alo_opcion%>.</label><div class="col-md-8"><div class="input-icon"><input name="tasas_opciones" type="text" class="form-control" id="tasas_opciones" placeholder="" onKeyUp="suma();" value="<%=tasas_opciones%>" maxlength="160"><div class="form-control-focus"> </div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">tasas</span></div></div></div><%end if%>
...
<%end if%>
<div class="col-md-12">
<div class="form-group form-md-line-input col-md-7"><label class="col-md-5 control-label" for="n_factura" >Nº Factura.</label><div class="col-md-7">
  <div class="input-icon"><input type="text" class="form-control" placeholder="<%'=n_factura_p%>" name="n_factura" id="n_factura" maxlength="8" value="<%=n_factura%>" <%if gen_fac<>"si" then%>disabled<%end if%> onKeyUp="despeja_f();"><div class="form-control-focus"> </div><i class="fa fa-sort-numeric-desc" aria-hidden="true"></i><span class="help-block">número correlativo <%=n_factura_p%></span></div></div></div>
...
<div class="col-md-6"><div class="form-group form-md-line-input col-md-9" ><label class="col-md-5 control-label" for="opp_p[<%=i%>]" ><%=ob_rs2("nom_opcion_prog")%>.</label><div class="col-md-7"><div class="input-icon"><input type="text" class="form-control" placeholder="" name="opp_p[<%=i%>]" id="opp_p<%=i%>" maxlength="160" value="<%if ob_rs2("opcion_pre_prog")<>0 then%><%=FormatNumber(ccur(ob_rs2("opcion_pre_prog")),2)%><%end if%>" onKeyUp="deschequea_op(<%=i%>);"><div class="form-control-focus"> </div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">precio opción</span></div></div></div><div class="md-checkbox col-md-3" style="margin-top:15px">
<input type="checkbox" id="oppc[<%=i%>]" class="c-check" value="<%=ob_rs2("id_programaOp")%>|<%=ob_rs2("nom_opcion_prog")%>" name="oppc[<%=i%>]" autocomplete="off" Checked class="md-check" onClick="desmarca_op(<%=i%>);">
<label for="oppc[<%=i%>]" onClick="desmarca_op(<%=i%>);"><span></span><span class="check"></span><span class="box"></span><i class="fa fa-arrow-right" aria-hidden="true"></i> sí</label></div></div>
<%end if
ob_rs3.close
ob_rs2.movenext
i=i+1
...
do while not ob_rs.eof
'VEMOS SI LO HEMOS SELECCIONADO
Sql2= "SELECT opcion_pre_curso, nom_opcion_curso, id_programaOp FROM programasOp WHERE id_programa ="&cod_prog&" AND trim(StrConv(nom_opcion_curso, 2, 1042))='"&ob_rs("opcion_esp")&"' AND (transfer is null or transfer<>'si') and id_itemSubOp<>0"
Set Ob_RS2 = Server.CreateObject("ADODB.Recordset")
Ob_RS2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then
op_curso_sel="si"
...
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="opc_p[<%=i%>]" id="opc_p<%=i%>" maxlength="160" value="<%if opcion_pre_curso_2<>0 then%><%=FormatNumber(ccur(opcion_pre_curso_2),2)%><%end if%>" onKeyUp="deschequea_op_c(<%=i%>);"><div class="form-control-focus"> </div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">precio opción</span></div></div></div><div class="md-checkbox col-md-3" style="margin-top:15px">
<input type="checkbox" id="opcc[<%=i%>]" class="c-check" value="<%=ob_rs("id_itemSubOp")%>|<%=ob_rs("opcion_esp")%>" name="opcc[<%=i%>]" autocomplete="off" <%if op_curso_sel="si"then%>Checked<%end if%> class="md-check" onClick="desmarca_op_c(<%=i%>);"><label for="opcc[<%=i%>]" onClick="desmarca_op_c(<%=i%>);"><span></span><span class="check"></span><span class="box"></span><i class="fa fa-arrow-right" aria-hidden="true"></i> sí</label>
</div></div>
<%ob_rs.movenext
op_curso_sel=""
opcion_pre_curso=""
...
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="opc_p[<%=i%>]" id="opc_p<%=i%>" maxlength="160" value="<%if ob_rs2("opcion_pre_curso")<>0 then%><%=FormatNumber(ccur(ob_rs2("opcion_pre_curso")),2)%><%end if%>" onKeyUp="deschequea_op_c(<%=i%>);"><div class="form-control-focus"> </div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">precio opción</span></div></div></div><div class="md-checkbox col-md-3" style="margin-top:15px">
<input type="checkbox" id="opcc[<%=i%>]" class="c-check" value="<%=ob_rs2("id_programaOp")%>|<%=ob_rs2("nom_opcion_curso")%>" name="opcc[<%=i%>]" autocomplete="off" <%if ob_rs2("id_itemsubop")<>0then%>Checked <%end if%> class="md-check" onClick="desmarca_op_c(<%=i%>);"><label for="opcc[<%=i%>]" onClick="desmarca_op_c(<%=i%>);"><span></span><span class="check"></span><span class="box"></span><i class="fa fa-arrow-right" aria-hidden="true"></i> sí</label>
</div></div>
<%end if
ob_rs3.close
ob_rs2.movenext
...
<!--CONCEPTOS--><div class="col-md-12"><!--<%'if gen_fac="si" then%> disabled<%'end if%>-->
<div class="campos_conceptos_<%=a%> form-group form-md-line-input campos_conceptos"><div class="col-md-12" style="border-top:2px solid #ddd;padding-top:20px;margin-bottom:20px;"><button class="inserta_concepto_<%=a%> btn green" ><i class="fa fa-plus" style="margin-right:8px"></i>Añadir Concepto</button></div>
<%i_conceptos=1
if cod_prog<>"" then
...
<%i_anticipos=1
if cod_prog<>"" then
Sql= "SELECT * FROM programasAnticipos WHERE id_programa="&cod_prog&" order by fecha desc"
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_connb,adOpenStatic, adCmdText
if not ob_rs.eof then
...
id_anticipo_b=ob_rs("id_programaanticipo")
anticipo_b=ob_rs("anticipo")
anticipo_pre_b=ob_rs("precio")
anticipo_ref_b=ob_rs("refpago")
anticipo_ref_pay_b=ob_rs("refpagopay")
...
<div class="col-md-8"><div class="input-icon"><input type="text" class="form-control" placeholder=""  name="anticipo_pre_[<%=i_anticipos%>]" id="anticipo_pre_<%=i_anticipos%>" value="<%=anticipo_pre_b%>" onkeyup="desmarca(<%=i_anticipos%>);" ><input type="hidden" class="form-control" placeholder=""  name="anticipo_pre_b_[<%=i_anticipos%>]" id="anticipo_pre_b_<%=i_anticipos%>" value="<%=anticipo_pre_b%>" ><div class="form-control-focus"></div><i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">precio.</span></div></div>
...
<div class="col-md-9"><div class="input-icon"><input type="text" class="form-control" placeholder=""  name="anticipo_fec_[<%=i_anticipos%>]" id="anticipo_fec_[<%=i_anticipos%>]" value="<%=anticipo_fec_b%>"><input type="hidden" class="form-control" placeholder=""  name="anticipo_ref_[<%=i_anticipos%>]" id="anticipo_ref_[<%=i_anticipos%>]" value="<%=anticipo_ref_b%>">
<input type="hidden" class="form-control" placeholder=""  name="anticipo_ref_pay_[<%=i_anticipos%>]" id="anticipo_ref_pay_[<%=i_anticipos%>]" value="<%=anticipo_ref_pay_b%>">
<div class="form-control-focus"></div><i class="fa fa-calendar" aria-hidden="true"></i><span class="help-block">dd/mm/aaaa</span></div></div>
</div></div>
...
<div class="form-group form-md-checkboxes"><div class="md-checkbox-inline"><div class="md-checkbox"><input type="checkbox" name="anticipo_pag_[<%=i_anticipos%>]" id="anticipo_pag_[<%=i_anticipos%>]" class="md-check" value="si" <%if pagado_b="si"then%>checked<%end if%> onClick="anticipa(<%=i_anticipos%>);"><label for="anticipo_pag_[<%=i_anticipos%>]" ><span></span><span class="check"></span><span class="box"></span> Pagado. </label><input type="hidden" class="form-control" placeholder=""  name="anticipo_pag_b_[<%=i_anticipos%>]" id="anticipo_pag_b_[<%=i_anticipos%>]" value="<%=pagado_b%>" ></div>
...
function borra2_<%=i_anticipos%>(){
m=0;
if(document.getElementById("anticipo_pag_[<%=i_anticipos%>]").checked==1){
m=parseFloat(document.getElementById("anticipo_pre_<%=i_anticipos%>").value.replace(',','.'));
vresult_rr=parseFloat(document.all.falta_pagar.value.replace(',','.')) +parseFloat(m)
document.all.falta_pagar.value=formatea(vresult_rr);
$('#anticipo_pre_<%=i_anticipos%>').val(0);
}//for	
}
</script>

<div class="modal fade" id="modal_borra_<%=i_anticipos%>" tabindex="-1" role="basic" aria-hidden="true"><!--MODAL-->
<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button><h4 class="modal-title"><i class="fa fa-exclamation-triangle rojo_ff0" aria-hidden="true"></i>  Eliminar Anticipo</h4></div><div class="modal-body"> ¿Seguro que quieres elimiar este concepto? <br>Comprueba siempre la cantidad que FALTA PAGAR. <br><br>En caso de desmarcar la casilla "eliminar" vuelve a introducir manualmente el precio.</div><div class="modal-footer"><button type="button" class="btn green" data-dismiss="modal" onClick="borra2_<%=i_anticipos%>();">Aceptar</button><button type="button" class="btn default" data-dismiss="modal" >Cancelar</button></div></div></div></div>
<script>tot_ant=<%=i_anticipos%></script><%
i_anticipos=i_anticipos+1
ob_rs.movenext
loop
else
i_anticipos=0
%><script>tot_ant=0;</script>
...
<div style="width:100%;max-width:550px;border:1px solid #32C5D2;text-align:center;margin:0 auto;padding:20px;margin-top:25px;margin-bottom:35px">
<div class="form-group form-md-line-input">
...
<i class="fa fa-eur" aria-hidden="true"></i><span class="help-block">importe restante a pagar</span></div></div></div>*por favor, cuadrar resultados manualmente también
</div>

<%if pr_observaciones="si" then%>
<div class="form-group form-md-line-input" style="margin-top:25px;"><label class="col-md-3 control-label" for="a_observaciones_crm" >Observaciones internas.</label><div class="col-md-6"><div class="input-icon"><textarea name="a_observaciones_crm" class="form-control" id="a_observaciones_crm" placeholder=""><%=a_observaciones_crm%></textarea><div class="form-control-focus"></div><i class="fa fa-quote-right" aria-hidden="true"></i><span class="help-block">observaciones para uso interno</span></div></div></div>
<%end if%>

</div>
<!--BOTONES--><div class="col-md-12 col-sm-12" style="margin-bottom:10px;padding:0px">
<div class="form-body"><div class="form-actions"><div class="row">
<div style="width:100%;max-width:750px;text-align:center;margin:0 auto;padding-left:0px">
<div class="col-md-offset-2 col-md-9">
<button type="button" class="btn green" value="enviar datos" onClick="Enviar_contabilidad();"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> modificar datos</button>

<script>
function despeja_f(){Ctrl=document.op_contabilidad.n_factura;$(Ctrl).css('border', 'none');$(Ctrl).css('border-bottom', '1px solid #ccc');}
function fact_ok(form) {
if (document.op_contabilidad.n_factura.value!=''){
var check_factura = /((F)[0-9][0-9][0-9][0-9](-)[0-9][0-9])/g;

factu = document.op_contabilidad.n_factura.value.toUpperCase();
Ctrl=document.op_contabilidad.n_factura;
...
function Enviar_contabilidad(){if (!fact_ok()) return;document.op_contabilidad.submit();}
</script>
...
<a href="../<%=r%>web/pr_pdf_factura.asp?id_programa=<%=cod_prog%>&s=crm&cod_cat=<%=cod_cat%>&proc=gest&cod=<%=cod%>" class="btn green btn-outline" ><i class="fa fa-print" aria-hidden="true"></i> imprimir factura </a></div>
...
<div class="form-group form-md-line-input">
<div class="col-md-12 col-md-offset-5" style="padding:0">
<div class="md-checkbox-inline" style="margin-top:25px" >
<div class="md-checkbox"><input name="mail_aviso" type="checkbox" class="md-check" id="mail_aviso2" value="si"><label class=" control-label" for="mail_aviso2"><i class="fa fa-share" aria-hidden="true" style="color:#ccc;margin-right:5px;"></i>Aviso alumno.<%if mail_aviso<>0 and mail_aviso<>"" then%>(<%=mail_aviso%>)<%end if%></label></div>
<div class="md-checkbox"><input name="completado" type="checkbox" class="md-check col-md-1" id="completado" value="si" <%if completado>0 then%>checked<%end if%>><label class="control-label" for="completado"><i class="fa fa-check" aria-hidden="true" style="color:#ccc;margin-right:5px;"></i>
Completado.<%if completado<>0 and completado<>"" then%>(<%=completado%>)<%end if%></label></div>
</div></div></div>
...
<div class="col-md-5">
<button type="button" onClick="Enviar_facturas<%=i_facturas%>();" class="btn green"  value="enviar datos" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> modificar datos</button>
<script>
function despeja<%=i_facturas%>(){Ctrl=document.facturas_<%=i_facturas%>.n_fac_<%=i_facturas%>;$(Ctrl).css('border', 'none');$(Ctrl).css('border-bottom', '1px solid #ccc');}
function fact_ok<%=i_facturas%>(form) {
if (document.facturas_<%=i_facturas%>.n_fac_<%=i_facturas%>.value!=''){
var check_factura<%=i_facturas%> = /((F)[0-9][0-9][0-9][0-9](-)[0-9][0-9])/g;
factu<%=i_facturas%> = document.facturas_<%=i_facturas%>.n_fac_<%=i_facturas%>.value.toUpperCase();
Ctrl=document.facturas_<%=i_facturas%>.n_fac_<%=i_facturas%>;
if(check_factura<%=i_facturas%>.test(factu<%=i_facturas%>) === true){return true;}else{$("#alerta").modal();$("#alerta_txt").text("El formato de factura introducido es incorrecto. Debe ser Fnnnn-aa. F, seguida de 4 dígitos, un guión y el año en formato 2 dígitos. Ej: F1234-18.");$(Ctrl).css('border', '2px solid #FF0000');return false;}
}else{return true;}}
function Enviar_facturas<%=i_facturas%>(){if (!fact_ok<%=i_facturas%>()) return;document.facturas_<%=i_facturas%>.submit();}
</script>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod_cat="&cod_cat))%>
<a href="<%=r%>alumnos/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod_cat="&cod_cat)%>" class="btn default"><i class="fa fa-reply" aria-hidden="true"></i> volver al listado</a>
</div>
...
ob_rs0.close%>
</div></div></div></div><%end if%>
<!--tab_Sup_2 DATOS GENERALES-->
<%if tip<>"eli" then%><div class="tab-pane <%=tab2%>" id="tab_2"><%end if%>
<%if tip="eli" then%><div class="tab-pane <%=tab2%> active" id="tab_2"><%end if%>
<div class="form-body"><div class="row"><div class="col-md-12"><div class="panel-group accordion scrollable" id="acordeon_c">
...
<div class="panel-heading"><h4 class="panel-title acordeon"><a class="accordion-toggle accordion-toggle-styled <%=tab22_2%>" data-toggle="collapse" data-parent="#acordeon_c" href="#acordeon_a2"><i class="fa fa-users" style="margin-right:4px"></i> Datos Familiares  / Otros datos / Datos Médicos</a></h4></div>
...
<div class="col-md-5">
<div class="form-group form-md-line-input">
<label class="col-md-4 control-label" for="a_p_nombre" >Enfermedades.</label>
<div class="col-md-8">
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="a_enfermedades" id="a_enfermedades"  maxlength="160" value="<%=a_enfermedades%>"><div class="form-control-focus"> </div><i class="fa fa-user-md" aria-hidden="true"></i><span class="help-block">enfermedades</span></div></div></div>
</div>
<div class="col-md-5">
<div class="form-group form-md-line-input">
<label class="col-md-4 control-label" for="a_colegio" >Medicamentos.</label>
<div class="col-md-8">
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="a_medicamentos" id="a_medicamentos"  maxlength="160" value="<%=a_medicamentos%>"><div class="form-control-focus"> </div><i class="fa fa-ambulance" aria-hidden="true"></i><span class="help-block">medicamentos</span></div></div></div>
</div><!--col-md-5-->
</div>
<div class="col-md-12">
<div class="col-md-5">
...
<div class="scroller" style="height: 200px;"><div class="panel-body documentos flotea">
<div class="col-md-12 col-sm-12" >
...
<input name="r" type="hidden" value="<%=r%>"><input name="rd" type="hidden" value="<%=rd%>"><input name="n" type="hidden" value="<%=n%>"><input name="ndos" type="hidden" value="<%=ndos%>"><input name="ntres" type="hidden" value="<%=ntres%>"><input name="tip" type="hidden" value="<%=tip%>"><input name="ncuat" type="hidden" value="<%=ncuat%>"><input name="edi" type="hidden" value="<%=edi%>">
...
<div class="modal-body" id="alerta_txt"> ¿Seguro que deseas eliminar este programa?<br><br>Se enviará a la papelera.</div>
...
<div class="modal fade" id="cancelarPrograma" tabindex="-1" role="basic" aria-hidden="true">
<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button><h4 class="modal-title"><i class="fa fa-exclamation-triangle gris_666" aria-hidden="true"></i>  Atención</h4></div>
<div class="modal-body" id="alerta_txt"> ¿Seguro que deseas cancelar este programa para este alumno?<br><br>Se enviará un mail al alumno.</div>
<div class="modal-footer"><button type="button" class="btn red " onClick="confirmarCancelaPrograma();">Confirmar</button><button type="button" class="btn green btn-green" data-dismiss="modal">Cancelar</button></div></div></div></div>

<script>function cancelarPrograma(){$('#cancelarPrograma').modal('show');}
function confirmarCancelaPrograma(){$('#cancelarPrograma').modal('hide');$('#spinner_back').show();$('#spinner').show();
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&cod_prog=<%=cod_prog%>&accion=cancel_prog&cod_cat=<%=cod_cat%>&check=<%=cod_prog%>&cod=<%=cod%>';}</script>

<div class="modal fade" id="descancelarPrograma" tabindex="-1" role="basic" aria-hidden="true">
<div class="modal-dialog"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button><h4 class="modal-title"><i class="fa fa-exclamation-triangle gris_666" aria-hidden="true"></i>  Atención</h4></div>
<div class="modal-body" id="alerta_txt"> ¿Seguro que deseas recuperar este programa cancelado para este alumno?<br><br>Se enviará un mail al alumno.</div>
<div class="modal-footer"><button type="button" class="btn red " onClick="confirmardesCancelaPrograma();">Confirmar</button><button type="button" class="btn green btn-green" data-dismiss="modal">Cancelar</button></div></div></div></div>

<script>function descancelarPrograma(){$('#descancelarPrograma').modal('show');}
function confirmardesCancelaPrograma(){$('#descancelarPrograma').modal('hide');$('#spinner_back').show();$('#spinner').show();
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&cod_prog=<%=cod_prog%>&accion=descancel_prog&cod_cat=<%=cod_cat%>&check=<%=cod_prog%>&cod=<%=cod%>';}</script>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&cod_prog=<%=cod_prog%>&accion=del_alum&cod_cat=<%=cod_cat%>&check=<%=cod_prog%>&cod=<%=cod%>&tip=<%=tip%>';}</script>
...
$("[name='pr_medico'],[name='con_examen'],[name='pr_cancelacion'],[name='pr_ajuste'],[name='gen_fac'],[name='a_permiso_medico'],[name='a_fuma'],[name='a_vegetariano'],[name='a_curso_ant'],[name='mail_aviso'],[name='completado']").bootstrapSwitch({onColor : 'info',onText: "Sí",offText: "No", size:"mini"});})
<%for a=1 to a-1%>
//CONCEPTO
...
$(".inserta_anticipo_<%=a%>").prop("disabled",false);
...
$(this).closest('.anticipo_lst_<%=a%>').remove();
tot_ant=$('.anticipo_lst_<%=a%>').length;suma();
...
//vresult_r = vtemp1 + vtemp2 + vtemp3 + vtemp4 + vtemp5 + vtemp6 + vtemp7 + vtemp8 + vtemp9 + vtemp10 + vtemp11 + vtemp12// - vtemp_c
vresult_r =vresult_r  - <%=pr_cancelacion_pr%> + vtemp8//vtemp_c
...
for (z=1; z<=tot_ant; z++){
if(document.getElementById("anticipo_pag_["+z+"]").checked==1){
if(document.getElementById("anticipo_pag_b_["+z+"]").value=="si" ){aj=document.getElementById("anticipo_pre_b_"+z+"").value;}
if(document.getElementById("anticipo_pag_b_["+z+"]").value==""){aj=document.getElementById("anticipo_pre_"+z+"").value;}

n_fix=document.getElementById("anticipo_pre_"+z+"").value.replace('.','')
n_fix = n_fix.toString();
n_fix= n_fix.replace(',','.');
n=parseFloat(n)+parseFloat(n_fix);

}
if(document.getElementById("anticipo_pag_["+z+"]").checked==0 && document.getElementById("anticipo_pag_b_["+z+"]").value=="si" ){}
//n=parseFloat(n)+parseFloat(document.getElementById("anticipo_pre_"+z+"").value.replace(',','.'));
}//for	

vresult_r=vresult_r-n
...
//ANTICIPOS
function desmarca(des){if(document.getElementById("anticipo_pag_["+des+"]").checked==1){document.getElementById("anticipo_pag_["+des+"]").checked=false}}
function deschequea_op(des){if(document.getElementById("oppc["+des+"]").checked==1){document.getElementById("oppc["+des+"]").checked=false;suma()}}
function deschequea_op_c(des){if(document.getElementById("opcc["+des+"]").checked==1){document.getElementById("opcc["+des+"]").checked=false;suma()}}
function desmarca_op(des){document.getElementById("oppc["+des+"]").onchange = function(){if(document.getElementById("oppc["+des+"]").checked==0){document.getElementById("opp_p"+des+"").value=''}
suma();}}
function desmarca_op_c(des){document.getElementById("opcc["+des+"]").onchange = function(){if(document.getElementById("opcc["+des+"]").checked==0){document.getElementById("opc_p"+des+"").value=''}
suma();}}

function anticipa(esto){
document.getElementById("anticipo_pag_["+esto+"]").onchange = function(){
if (document.getElementById("anticipo_["+esto+"]").value!=''){	
	n=0;
	aj=0;
//if(document.getElementById("anticipo_pag_["+esto+"]").checked==1){
	for (z=1; z<=tot_ant; z++){
		if(document.getElementById("anticipo_pag_["+z+"]").checked==1 && esto==z ){
			
			if(document.getElementById("anticipo_pag_b_["+z+"]").value=="si"  && esto==z ){
				if(esto==z){aj=0;}else{aj=document.getElementById("anticipo_pre_b_"+z+"").value;}
				}
			if(document.getElementById("anticipo_pag_b_["+z+"]").value=="" && esto==z){aj=0;}
			//n_fix=parseFloat(document.getElementById("anticipo_pre_"+z+"").value.replace('.',''));
			n_fix=document.getElementById("anticipo_pre_"+z+"").value.replace('.','')
			n_fix = n_fix.toString();
			n_fix= n_fix.replace(',','.');
			n=parseFloat(n)+n_fix-parseFloat(aj);

		}
		if(document.getElementById("anticipo_pag_["+z+"]").checked==0 && document.getElementById("anticipo_pag_b_["+z+"]").value=="si" && esto==z ){
			n_fix=document.getElementById("anticipo_pre_"+z+"").value.replace('.','')
			n_fix = n_fix.toString();
			n_fix= n_fix.replace(',','.');
			n=parseFloat(n)-n_fix;
			
		}
		if(document.getElementById("anticipo_pag_["+z+"]").checked==0 && document.getElementById("anticipo_pag_b_["+z+"]").value=="" && esto==z){
			//aj=document.getElementById("anticipo_pre_b_"+z+"").value;
			n_fix=document.getElementById("anticipo_pre_"+z+"").value.replace('.','')
			n_fix = n_fix.toString();
			n_fix= n_fix.replace(',','.');
			n=parseFloat(n)-n_fix;
...
vresult_r=document.all.falta_pagar.value.replace('.','').replace(',','.');
document.all.falta_pagar.value=formatea(vresult_r-n);
//}//Checked=1	
}else{$("#alerta").modal();$("#alerta_txt").text("Debes indicar un concepto en el anticipo "+esto+" para poder realizar operaciones.");document.getElementById("anticipo_pag_["+esto+"]").checked=false}
}
}
