...
<%if cod<>""then
Sql= "SELECT * from rel where id_destino="&cod&" and id_archivo<>0" '[IMAGENES VIDEOS TOTALES]
...
ob_rs.close
Sql= "SELECT * from destinos where id_destino="&cod&""     '[RECOGEMOS DATOS SI MODIFICACION]
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
...
ob_rs.close
Sql= "SELECT * from rel where id_destino="&cod&""
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
i=1
...
<h1>Gestión Destinos | <%if cod<>""then%>Modificar<%else%>Crear<%end if%> </h1>
<ol class="breadcrumb">
...
<a href="<%=r%>destinos/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>">Listado Destinos</a></li>
<li class="active">Gestión Destinos</li>
...
<li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat))%>
<a href="<%=r%>destinos/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>"><i class="icon-list"></i> Listado Destinos
<%if num_dt<>""then%><span class="baddge badge badge-default"> <%=num_dt%> </span><%else%><span class="baddge badge badge-warning"> 0 </span><%end if%></a></li>
<li class="active"><a href="#"><i class="icon-pin"></i> Gestión Destino</a></li>
...
<li><a data-toggle="modal" data-target="#externo1"><i class="icon-list"></i> Listar Programas <%if num_it<>""then%><span class="baddge badge badge-default"> <%=num_it%> </span><%else%><span class="baddge badge badge-warning"> 0 </span><%end if%></a></li>
</ul>
</nav>
</div><%end if%>
...
<div class="caption"><i class="icon-note font-green"></i><span class="caption-subject font-green sbold uppercase">Gestión Destino <%if titulo_esp<>""then%>| <%=titulo_esp%><%end if%></span></div>
...
<label class="col-md-4 control-label" for="titulo_esp" >Destino (país).<span class="required">*</span></label>
<div class="col-md-8">
...
<i class="fa fa-globe" aria-hidden="true"></i><span class="help-block">debes introducir un país</span></div></div></div>
<!--IMAGEN-->
...
<div class="col-md-4">
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="orden_general" id="orden_general" maxlength="160" value="<%if orden_general<>0 and cod<>"" then%><%=orden_general%><%else%><%=num_dt+1%><%end if%>"><div class="form-control-focus"></div>
<i class="fa fa-sort-numeric-desc" aria-hidden="true"></i><span class="help-block">orden destino</span></div>
</div>
</div>
...
<button type="button" class="btn green" onclick="Enviar_destinos(this.form);" value="enviar datos" language="javascript"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> modificar datos</button>
