...
mas_texto=request("mas_texto")
pr_team=request("pr_team")
id_formulario=request("id_formulario")
If IsNumeric(id_formulario) Then id_formulario=id_formulario else id_formulario=0 end if
if id_formulario="" then id_formulario=0 end if
...
if ntres="tn"then url_esp="test-nivel-ingles" end if
if ntres="ck"then url_esp="politica-cookies" end if
if ntres="hm" or ntres="ck" then orden_general=9999999 end if
...
Sql= "SELECT url_esp FROM generales WHERE url_esp ='"&url_esp&"' and id_general <> "&cod&""   
else
Sql= "SELECT url_esp FROM generales WHERE url_esp ='"&url_esp&"'"   
end if
...
mySQL="INSERT INTO generales (titulo_esp,texto_esp,url_esp,meta_title,meta_key,meta_desc,visible,fecha_in,destacado,orden,tipo,mas_texto,texto_esp2,pr_team,id_formulario)" 				
	...
	mySQL=mySQL & "" & orden_general & ","
	mySQL=mySQL & "'" & tipo & "',"
	mySQL=mySQL & "'" & mas_texto & "',"
	mySQL=mySQL & "'" & texto_esp2 & "',"
	mySQL=mySQL & "'" & pr_team & "',"
	mySQL=mySQL & "" & id_formulario & ""
	mySQL=mySQL & ");"
set ob_rs=Ob_Conn.execute(mySQL)
set ob_rs = Ob_Conn.execute("select MAX(id_general) from generales") 
cod = ob_rs(0)
ob_rs.close

end if                                                            '[FIN OPCION INSERTAR]
if op="up"then                                                    '[OPCION MODIFICAR]
mySQL="UPDATE generales SET "
...
mySQL=mySQL & "orden = " & orden_general & ","
mySQL=mySQL & "tipo = '" & tipo & "',"
mySQL=mySQL & "mas_texto = '" & mas_texto & "',"
mySQL=mySQL & "texto_esp2 = '" & texto_esp2 & "',"
mySQL=mySQL & "pr_team = '" & pr_team & "',"
mySQL=mySQL & "id_formulario = " & id_formulario & ""
mySQL=mySQL & " WHERE id_general = " & cod & "" 
set ob_rs=Ob_Conn.execute(mySQL)
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_general, archivos.id_archivo, archivos.campo from rel, archivos where rel.id_general="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and archivos.campo ='"&campo&"'"                    
Ob_Command.CommandText = Sql
...
id_archivo = ob_rs(0)
tipo_rel=tipo
mySQL="INSERT INTO rel (id_general,id_archivo,tipo_rel,fecha_in,fecha_mod)" '[RELACIONAMOS IMAGEN CON WEB]
	mySQL=mySQL & "VALUES ("
	mySQL=mySQL & "" & cod & ","
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_general, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_general="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo "                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
do while not ob_rs3.eof
...
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_general, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_general="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and (archivos.campo <> 'archivo_1' and archivos.campo <> 'archivo_2' and archivos.campo <> 'archivo_17' and archivos.campo <> 'archivo_18' and archivos.campo <> 'archivo_19' and archivos.campo <> 'archivo_20') order by archivos.orden"                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
ord=3
...
mySQL="DELETE FROM rel where id_general =" & cod & " and id_pag_sup <> 0" 'ELIMINAMOS PRIMERO
set ob_rs=Ob_conn.execute(mySQL)
if id_pag_sup<>0then                                              '[RELACIONAMOS EN REL ID_PROVEEDOR]
mySQL="INSERT INTO rel (id_pag_sup,id_general)" 				
	mySQL=mySQL & "VALUES ("         '[INSERTAMOS]
	mySQL=mySQL & "" & id_pag_sup & ","
	mySQL=mySQL & "'" & cod & "'"
	mySQL=mySQL & ");"
set ob_rs=Ob_Conn.execute(mySQL)
end if  

Sql= "SELECT id_general, titulo_esp from generales where id_general="&cod&" "                    
Ob_Command.CommandText = Sql
SET Ob_RS = Ob_Command.Execute ()
if not ob_rs.eof then
...
Sql= "SELECT url_esp,id_general from generales"                            '[COMENZAMOS REESCRITURA]
Ob_Command.CommandText = Sql
SET Ob_RS = Ob_Command.Execute ()
if not ob_rs.eof then
set fso = createobject("scripting.filesystemobject")
Set act = fso.CreateTextFile(server.mappath(rd&"mapas_url/generales.txt"), true)
do while not ob_rs.eof
url = ob_rs("url_esp")
act.WriteLine ""&limpia_url(url)&" "&ob_rs("id_general")&"" 
ob_rs.movenext
loop
act.Close
end if
