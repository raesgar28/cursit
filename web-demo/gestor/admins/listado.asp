...
<div class="breadcrumbs"><h1>Usuarios CRM | Gestión Usuarios CRM</h1><ol class="breadcrumb"><li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n=dash&ndos="))%><a href="<%=r%>dash/index.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n=dash&ndos=")%>">Escritorio</a></li><li><a href="#">Listado <%if ntres="ag" then%>Agentes<%else%>Usuarios<%end if%></a></li>
</ol></div><div class="row"><div class="col-md-12"><div class="portlet light portlet-fit portlet-datatable bordered">
<div class="portlet-title">
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Usuarios CRM registradaos</span></div>
<div class="actions">
<div class="btn-group">
...
<ul class="dropdown-menu pull-right"><li><a href="javascript:habilitadoTodas();" ><i class="fa fa-circle-o" aria-hidden="true"></i> Habilitado</a></li>
<li><a href="javascript:habilitadoNoTodas();" ><i class="fa fa-ban" aria-hidden="true"></i> No habilitado</a></li>
<li class="divider"></li><li><a href="javascript:eliminarTodas();" ><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul></div>
<div class="btn-group">
...
<%Sql= "SELECT nombre, apellidos, fecha_in, id_admin,mail, habilitado, principal FROM admins"   
...
</label></th><th>Nombre</th><th> Habilitado </th><th> Email </th><th> Fecha Registro </th><th class="txt_centro"> acc. </th></tr></thead>
<tbody><%do while not ob_rs.eof%><tr><td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_admin")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td><%=ob_rs("apellidos")%>, <%=ob_rs("nombre")%><%if ob_rs("principal")="si"then%><i class="fa fa-user-circle-o" aria-hidden="true" style="margin-left:12px;color:#999"><%end if%></i>
</td><td><%if ob_rs("habilitado")="si"then%><i class="fa fa-check f_green" aria-hidden="true"></i><span class="f_white_p">Sí</span><%else%><i class="fa fa-times f_red" aria-hidden="true"></i><span class="f_white_p">No</span><%end if%></td>
<td><%=ob_rs("mail")%></td>
<td><%if ob_rs("fecha_in")<>""then%><%=FormatDateTime(cdate(ob_rs("fecha_in")),2)%><%end if%></td>
...
<ul class="dropdown-menu pull-right" id="acciones" >
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_admin")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_admin"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<%if ob_rs("principal")<>"si" or ISNULL (ob_rs("principal")) then%>
<li class="divider"></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_admin")&"&accion=hab"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_admin")&"&accion=hab")%>&" ><i class="fa fa-eye" aria-hidden="true"></i> Habilitado</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_admin")&"&accion=hab_no"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_admin")&"&accion=hab_no")%>&"  ><i class="fa fa-eye-slash" aria-hidden="true"></i> No habilitado</a></li>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_admin")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
<%end if%>
</ul>
</div></td></tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_admin")%>&accion=del';}</script>
