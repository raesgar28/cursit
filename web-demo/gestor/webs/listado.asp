...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | WEBS REGISTRADAS</span></div>
<div class="actions" >
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_web")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td> <%=ob_rs("titulo_esp")%> </td>
<td>
<%Sql2= "SELECT * from rel where id_web = "&ob_rs("id_web")&" and id_division <> 0 "   
Set ob_rs2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_conn,adOpenStatic, adCmdText
if not ob_rs2.eof then
do while not ob_rs2.eof
id_division=ob_rs2("id_division")
if id_division<>""then
Sql3= "SELECT * from divisiones where id_division = "&id_division&" "   
Set ob_rs3 = Server.CreateObject("ADODB.Recordset")
ob_rs3.Open Sql3, ob_conn,adOpenStatic, adCmdText
if not ob_rs3.eof then
Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n=ad&ndos=dn&ntres="&ntres&"&ncuat="&ncuat&"&cod="&id_web))%>
<a href="<%=r%>divisiones/gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n=ad&ndos=dn&ntres="&ntres&"&ncuat="&ncuat&"&cod="&id_web)%>" class="enlace_tabla_in"><%=ob_rs3("titulo_esp")%><i class="fa fa-pencil-square-o margin-left-12" style="color:#ddd" aria-hidden="true"></i></a>
<%end if
ob_rs3.close
end if
ob_rs2.movenext%><br />
...
<td><%if ob_rs("principal")="si"then%><i class="fa fa-check f_green" aria-hidden="true"></i><span class="f_white_p">Sí</span><%else%><span class="f_white_p">No</span><%end if%></td>
...
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_web")))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_web"))%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_web")&"&accion=vis"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_web")&"&accion=vis")%>&" ><i class="fa fa-eye" aria-hidden="true"></i> Visible</a></li>
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_web")&"&accion=vis_no"))%>
<li><a href="acciones.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&check="&ob_rs("id_web")&"&accion=vis_no")%>&"  ><i class="fa fa-eye-slash" aria-hidden="true"></i> No visible</a></li>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_web")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_web")%>&accion=del';}
</script>
