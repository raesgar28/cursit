...
mySQL="DELETE FROM formularios where id_formulario =" & pks(i) & " "
set ob_rs=Ob_Conn.execute(mySQL)
mySQL="DELETE FROM formulariosCampos where id_formulario =" & pks(i) & " "
set ob_rs=Ob_Conn.execute(mySQL)
mySQL="DELETE FROM formulariosRes where id_formulario =" & pks(i) & " "
set ob_rs=Ob_Connb.execute(mySQL)
mySQL3="UPDATE generales SET "
mySQL3=mySQL3 & "id_formulario =  0 "
mySQL3=mySQL3 & " WHERE id_formulario = " & pks(i) & "" 
set ob_rs=Ob_Conn.execute(mySQL3)
end if
if accion="del_b"then 
mySQL="DELETE FROM formulariosRes where id_respuesta =" & pks(i) & " "
set ob_rs=Ob_Connb.execute(mySQL)
end if
Next
'[REDIRECCIONAMOS]
