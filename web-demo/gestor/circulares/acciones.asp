...
if n_letter="si"then
Sql= "SELECT u_email, n_letter, u_nombre, u_apellidos  FROM usuarios WHERE id_usuario="&id_usuario&" and n_letter='si'"
else
Sql= "SELECT u_email, n_letter, u_nombre, u_apellidos FROM usuarios WHERE id_usuario="&id_usuario&""
end if
Set Ob_RS = Server.CreateObject("ADODB.Recordset")
Ob_RS.Open Sql, Ob_Connb,adOpenStatic, adCmdText
if not ob_rs.eof then
u_email=ob_rs("u_email")
n_letter_c=ob_rs("n_letter")
%>Usuario (<%=id_usuario%>): <%=ob_rs("u_nombre")%>&nbsp;<%=ob_rs("u_apellidos")%> <br> <%
else
falla_usuario="si"
end if
ob_rs.close
...
if mail_1<>mail_4 and  mail_1<>"" then'and mail_1<>mail_2 and mail_1<>mail_3 and
if n_letter="si" and n_letter_c="si" then
...
if int(id_usuario_ctrl)<>int(id_usuario) then
...
if accion="env_mail_lst" or accion="env_mail" then
'if Err = 0 then
