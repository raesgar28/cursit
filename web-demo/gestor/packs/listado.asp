...
<div class="caption"><i class="icon-settings font-green"></i><span class="caption-subject font-green sbold uppercase">Listado | Packs registrados</span></div>
<div class="actions">
<div class="btn-group">
...
<%Sql= "SELECT id_pack, titulo_esp, texto_esp, fecha_in FROM packs WHERE id_categoria ="&cod_cat&""   
Set ob_rs = Server.CreateObject("ADODB.Recordset")
...
<tr>
<td><label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" class="checkboxes" value="<%=ob_rs("id_pack")%>" onClick="if (this.checked) marca(1); else marca('-1');" name="check"  autocomplete="off"><span></span></label></td>
<td><%=ob_rs("titulo_esp")%></td>
<td>
<%Sql2= "SELECT id_item FROM packs_rel WHERE id_pack="&ob_rs("id_pack")&" AND id_item<>0"   
...
<td>
<%Sql2= "SELECT id_categoria FROM packs_rel WHERE id_pack="&ob_rs("id_pack")&" AND id_categoria<>0"   
Set ob_rs2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then
do while not ob_rs2.eof
...
<td>
<%Sql2= "SELECT id_seccion FROM packs_rel WHERE id_pack="&ob_rs("id_pack")&" AND id_seccion<>0"   
Set ob_rs2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then
do while not ob_rs2.eof
...
<td><%Sql2= "SELECT id_destino FROM packs_rel WHERE id_pack="&ob_rs("id_pack")&" AND id_destino<>0"   
Set ob_rs2 = Server.CreateObject("ADODB.Recordset")
ob_rs2.Open Sql2, ob_connb,adOpenStatic, adCmdText
if not ob_rs2.eof then
do while not ob_rs2.eof
...
<ul class="dropdown-menu pull-right" id="acciones" >
<%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r=" & r & "&rd=" & rd &"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_pack")&"&cod_cat="&cod_cat))%>
<li><a href="gestion.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat&"&cod="&ob_rs("id_pack")&"&cod_cat="&cod_cat)%>" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar</a></li>
<li class="divider"></li>
<li><a href="javascript:eliminarUna<%=i%>(<%=ob_rs("id_pack")%>);"><i class="fa fa-trash-o" aria-hidden="true"></i> Eliminar</a></li>
</ul>
</div>
</td>
</tr>
...
location.href='acciones.jpk2?r=<%=r%>&rd=<%=rd%>&n=<%=n%>&ndos=<%=n2%>&ntres=<%=n3%>&ncuat=<%=n4%>&check=<%=ob_rs("id_pack")%>&accion=del&cod_cat=<%=cod_cat%>';}</script>
