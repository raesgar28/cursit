...
orden_sec=request("orden_sec")
en_menu=request("en_menu")
If IsNumeric(orden_sec) Then orden_sec=orden_sec else orden_sec=0 end if
if meta_title<>""then 
...
if op="up" then
Sql= "SELECT titulo_esp FROM secciones WHERE titulo_esp= '"&titulo_esp&"' and id_seccion <> "&cod&" "
else
Sql= "SELECT titulo_esp FROM secciones WHERE titulo_esp= '"&titulo_esp&"'"
end if
...
if op="up" then
Sql= "SELECT noticias.url_esp,generales.url_esp,items.url_esp,itemsSub.url_esp,categorias.url_esp,secciones.url_esp,subsecciones.url_esp,destinos.url_esp,ciudades.url_esp from noticias,generales,items,itemsSub,categorias,secciones,subsecciones,destinos,ciudades WHERE ( (secciones.url_esp ='"&url_esp&"' or secciones.titulo_esp= '"&titulo_esp&"') and id_seccion <> "&cod&") or (generales.url_esp = '"&url_esp&"' or items.url_esp= '"&url_esp&"' or itemsSub.url_esp = '"&url_esp&"' or categorias.url_esp = '"&url_esp&"' or noticias.url_esp= '"&url_esp&"' or subsecciones.url_esp = '"&url_esp&"' or destinos.url_esp = '"&url_esp&"' or ciudades.url_esp = '"&url_esp&"' ) "   
else
...
mySQL="INSERT INTO secciones (titulo_esp,texto_esp,url_esp,meta_title,meta_key,meta_desc,visible,fecha_in,disponible,destacado,orden,en_menu)" 				
	...
	mySQL=mySQL & "" & orden_sec & ","
	mySQL=mySQL & "'" & en_menu & "'"
	mySQL=mySQL & ");"
set ob_rs=Ob_Conn.execute(mySQL)
set ob_rs = Ob_Conn.execute("select MAX(id_seccion) from secciones") 
cod = ob_rs(0)
ob_rs.close
if id_categoria<>""then                                              '[RELACIONAMOS EN REL ID_WEB <> ID_CATEGORIA]
cadena_rel = split(id_categoria, ",")
for i=0 to ubound(cadena_rel)
mySQL="INSERT INTO rel (id_categoria,id_seccion)" 				
	mySQL=mySQL & "VALUES ("         '[INSERTAMOS]
...
mySQL=mySQL & "orden = " & orden_sec & ","
mySQL=mySQL & "en_menu = '" & en_menu & "'"
mySQL=mySQL & " WHERE id_seccion = " & cod & "" 
set ob_rs=Ob_Conn.execute(mySQL)
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_seccion, archivos.id_archivo, archivos.campo from rel, archivos where rel.id_seccion="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and archivos.campo ='"&campo&"'"                    
Ob_Command.CommandText = Sql
...
id_archivo = ob_rs(0)
tipo_rel=tipo
mySQL="INSERT INTO rel (id_seccion,id_archivo,tipo_rel,fecha_in,fecha_mod)" '[RELACIONAMOS IMAGEN CON WEB]
	mySQL=mySQL & "VALUES ("
	mySQL=mySQL & "" & cod & ","
...
campo="archivo_"&i
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_seccion, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_seccion="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo "                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
do while not ob_rs3.eof
...
Sql= "SELECT rel.id_archivo,rel.id_rel, rel.id_seccion, archivos.id_archivo, archivos.campo, archivos.orden from rel, archivos where rel.id_seccion="&cod&" and rel.id_archivo<>0 and archivos.id_archivo = rel.id_archivo and (archivos.campo <> 'archivo_1' and archivos.campo <> 'archivo_2' and archivos.campo <> 'archivo_17' and archivos.campo <> 'archivo_18' and archivos.campo <> 'archivo_19' and archivos.campo <> 'archivo_20') order by archivos.orden"                           
Ob_Command.CommandText = Sql
SET Ob_RS3 = Ob_Command.Execute ()
if not ob_rs3.eof then
ord=3
...
if id_categoria<>""then                                              '[RELACIONAMOS EN REL ID_WEB <> ID_CATEGORIA]
mySQL="DELETE FROM rel where id_seccion =" & cod & " and id_categoria <> 0 and id_subseccion=0 and id_item=0" 'ELIMINAMOS PRIMERO
set ob_rs=Ob_conn.execute(mySQL)
cadena_rel = split(id_categoria, ",")
for i=0 to ubound(cadena_rel)
mySQL="INSERT INTO rel (id_categoria,id_seccion)" 				
...
Sql= "SELECT id_seccion, url_esp from secciones"                            '[COMENZAMOS REESCRITURA]
Ob_Command.CommandText = Sql
SET Ob_RS = Ob_Command.Execute ()
if not ob_rs.eof then
set fso = createobject("scripting.filesystemobject")
Set act = fso.CreateTextFile(server.mappath(rd&"mapas_url/secciones.txt"), true)
do while not ob_rs.eof
url = ob_rs("url_esp")
act.WriteLine ""&limpia_url(url)&" "&ob_rs("id_seccion")&"" 
ob_rs.movenext
loop
act.Close
end if
