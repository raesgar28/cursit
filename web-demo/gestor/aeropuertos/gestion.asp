...
<%if cod<>""then
Sql= "SELECT * from aeropuertos where id_aeropuerto="&cod&""     '[RECOGEMOS DATOS SI MODIFICACION]
Set ob_rs = Server.CreateObject("ADODB.Recordset")
ob_rs.Open Sql, ob_conn,adOpenStatic, adCmdText
if not ob_rs.eof then
ciudad=ob_rs("ciudad")
texto_esp=ob_rs("texto_esp")
nacional=ob_rs("nacional")
end if
ob_rs.close
...
<h1>Gestión Aeropuerto | <%if cod<>""then%>Modificar<%else%>Crear<%end if%> </h1>
<ol class="breadcrumb">
...
<a href="<%=r%>aeropuertos/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>">Listado Aeropuertos</a></li>
<li class="active">Gestión Aeropuerto</li>
</ol>
</div>
...
<li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat))%>
<a href="<%=r%>aeropuertos/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>"><i class="icon-list"></i> Listado Aeropuertos
<%if num_ar<>""then%><span class="baddge badge badge-default"> <%=num_ar%> </span><%else%><span class="baddge badge badge-danger"> 0 </span><%end if%></a></li>
<li class="active"><a href="#"><i class="icon-pin"></i> Gestión Aeropuerto</a></li>
</ul>
</nav>
</div>
...
<div class="caption"><i class="icon-note font-green"></i><span class="caption-subject font-green sbold uppercase">Gestión Aeropuerto <%if titulo_esp<>""then%>| <%=titulo_esp%><%end if%></span></div>
</div>
...
<%if ms="ko"then%><div class="alert alert-danger"><button class="close" data-close="alert"></button><i class="fa fa-exclamation-triangle" aria-hidden="true"></i> El aeropuerto ya se encuentra registrad.</div><%end if%>
...
<div class="col-md-12">
<div class="col-md-6">
<div class="form-group form-md-line-input" id="ciudad_a">
<label class="col-md-4 control-label" for="ciudad" >Ciudad.<span class="required">*</span></label>
<div class="col-md-8">
<div class="input-icon"><input type="text" class="form-control" placeholder="" name="ciudad" id="ciudad" onKeyUp="resetea_error('ciudad_a');" maxlength="160" value="<%=ciudad%>"><div class="form-control-focus"> </div>
<i class="fa fa-plane" aria-hidden="true"></i><span class="help-block">debes introducir un nombre de ciudad</span></div></div></div>
</div><!--col-md-6-->
<div class="col-md-6">
<div class="form-group form-md-checkboxes">
<div class="col-md-7">
<div class="md-checkbox-list">
<div class="md-checkbox"><input name="nacional" type="checkbox" class="md-check" id="nacional" value="si" <%if cod=""then%>checked="checked"<%end if%> <%if nacional="si"then%>checked="checked"<%end if%>>
<label for="nacional">Nacional </label></div>
</div>
</div>
</div>
</div><!--col-md-6-->
...
<button type="button" class="btn green" onclick="Enviar_aeropuertos(this.form);" value="enviar datos" language="javascript"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> modificar datos</button>
...
<a href="<%=r%>aeropuertos/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>" class="btn default"><i class="fa fa-reply" aria-hidden="true"></i> volver al listado</a>
</div>
</div>
</div>
</div><!--form-body-->
