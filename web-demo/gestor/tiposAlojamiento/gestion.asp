...
<%if cod<>""then
Sql= "SELECT * from alo_tipo where id_alo_tipo="&cod&""     '[RECOGEMOS DATOS SI MODIFICACION]
...
<a href="<%=r%>tiposAlojamiento/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>">Listado Tipos de alojamiento</a></li>
...
<li><%Encryption_Key = mid(ReadKeyFromFile(Encryption_KeyLocation),1,Len("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat))%>
<a href="<%=r%>tiposAlojamiento/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>"><i class="icon-list"></i>  Tipos  alojamiento
<%if num_al<>""then%><span class="baddge badge badge-default"> <%=num_al%> </span><%else%><span class="baddge badge badge-danger"> 0 </span><%end if%></a></li>
...
<div class="caption"><i class="icon-note font-green"></i><span class="caption-subject font-green sbold uppercase">Gestión Tipo Alojamiento
...
<label class="col-md-4 control-label" for="ciudad" > Tipo de alojamiento.<span class="required">*</span></label>
<div class="col-md-8">
...
<i class="fa fa-home" aria-hidden="true"></i><span class="help-block">i.e. hotel</span></div></div></div>
</div><!--col-md-6-->
<%if cod<>""then
ordena=orden
else
...
<a href="<%=r%>tiposAlojamiento/listado.jpk2?enc=<%=EnCrypt("?r="&r&"&rd="&rd&"&n="&n&"&ndos="&ndos&"&ntres="&ntres&"&ncuat="&ncuat)%>" class="btn default"><i class="fa fa-reply" aria-hidden="true"></i> volver al listado</a>
</div>
</div>
</div>
</div><!--form-body-->
